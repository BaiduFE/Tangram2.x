 ...
 * @function 
 * @name baidu.query().before()
 * @grammar baidu.query(args).before(content[, content])
 ...
 * @function 
 * @name baidu.query().before()
 * @grammar baidu.query(args).before(fn)
 * @param {function} fn 支持一个函数作为参数，函数最终需要返回一个HTMLString|Element|TangramDom
 * @return {TangramDom} 返回之前匹配元素的TangramDom对象
 * @example baidu.query("<div>").before(function(index,html));
 */
baidu.query.extend({
    before: function(){
