 ...
 * @function 
 * @name baidu.query().off()
 * @grammar baidu.query(args).off(events[,selector],fn)
 ...
 * @function 
 * @name baidu.query().off()
 * @grammar baidu.query(args).off(eventMap[,selector],fn)
...
baidu.query.extend({
    off: function( events, selector, fn ){
        var eb = baidu.event.eventBase._core, me = this;
        if( !events )
