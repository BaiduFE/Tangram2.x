 ...
 * @function
 * @name baidu.query().prepend()
 * @grammar baidu.query(args).prepend(content1[, content2])
 ...
 * @function
 * @name baidu.query().prepend()
 * @grammar baidu.query(args).prepend(fn)
 * @param {function} fn 支持一个函数作为参数，函数最终需要返回一个HTMLString|Element|TangramDom.
 * @return {TangramDom} 返回之前匹配元素的TangramDom对象.
 * @example baidu.query("<div>").prepend(function(index,html))
 */
baidu.query.extend({
    prepend: function() {
