...
test('clone normal dom', function(){
    var div = new Elements('div'),
        ...
        tang;
    node.id = 'tangId';
    tang = baidu.query(div.get()).clone();
...
test('clone a text node', function(){
    var txt = document.createTextNode('hello world'),
        tang = baidu.query(txt).clone(true, true);
...
test('clone event but not deep', function(){
    ...
    ua.importsrc('baidu.query.on', function(){
        ...
        baidu.query(div.get()).on('click', function(evt){
            ok(true, 'div click event trigger');
        });
        baidu.query(span.get()).on('mouseover', function(evt){
            ok(false, 'span mouseover event trigger');
        });
        baidu.query(span_child.get()).on('mouseout', function(evt){
            ok(false, 'span child mouseout event trigger');
        });
        tang = baidu.query(div.get()).clone(true);
        ...
        span_child.dispose();
        start();
    }, 'baidu.query.on', 'baidu.query.clone');
});
...
test('clone all', function(){
    ...
    tang = baidu.query(div.get()).clone(true, true);
    ...
    ua.importsrc('baidu.query.trigger', function(){
        baidu.query(div.get()).trigger('click');
        baidu.query(span.get()).trigger('mouseover');
        baidu.query(span_child.get()).trigger('mouseout');
        ...
        div.dispose();
        start();
    }, 'baidu.query.trigger', 'baidu.query.clone');
});
...
test('clone object, textarea, radio, option, script', function(){
    $.each('object,textarea,script'.split(','), function(index, item){
        var c = new Elements(item),
            node = c.get();
        node.id = 'tangId-' + item;
        var tang = baidu.query(node).clone(true, true);
    ...
    tang = baidu.query(node).clone(true, true);
    ok(tang.get(0).id === node.id, 'same id: (' + node.id + ')');
    ...
    tang = baidu.query(opt).clone(true, true);
...
test('clone once event', function(){
    ...
    var div = new Elements('div'),
        cloneNode;
    stop();
    ua.importsrc('baidu.query.one', function(){
        ...
        div.dispose();
        start();
    }, 'baidu.query.one', 'baidu.query.clone');
});
...
test('clone delegate event', function(){
    ...
    ua.importsrc('baidu.query.delegate', function(){
        ...
        cloneNode = null;
        start();
    }, 'baidu.query.delegate', 'baidu.query.clone');
});
