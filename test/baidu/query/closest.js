...
test('单个元素', function () {
	useTangramDom(html, function ($dom) {
		var target = baidu.query('.item-a').closest('ul');
		equal(target[0].className, 'level-2');

		target = baidu.query('.item-a').closest('li');
...
test('多个元素', function () {
	useTangramDom(html, function ($dom) {
		var target = baidu.query('.item-1, .item-c').closest('ul');
...
test('.closest( context )重载', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query('.item-1').closest('body', document);
		equal(target.length, 1, 'find body');
		equal(target[0].tagName, 'BODY');
		var target = baidu.query('.item-1').closest('body', wrapper);
