...
test('baidu.query(args).detach()', function(){
    var ele = new Elements('div'),
        span = new Elements('span', true);
    ele.get().appendChild(span.get());
    baidu.query(ele.get()).detach();
    baidu.query(span.get()).detach();
...
test('baidu.query(args).detach(selector)', function(){
    ...
    baidu.query(container.get().getElementsByTagName('*')).detach('div');
...
test('event', function(){
    ...
    ua.importsrc('baidu.query.on', function(){
        ...
        baidu.query(span.get()).on('click', function(){
            ok(true, 'span event trigger');
        });
        baidu.query(div.get()).on('mouseover', function(){
            ok(true, 'div event trigger');
        });
        baidu.query(div.get()).detach();
        ua.fireMouseEvent(span.get(), 'click');
        ua.fireMouseEvent(div.get(), 'mouseover');
        start();
    }, 'baidu.query.on', 'baidu.query.detach');
});
