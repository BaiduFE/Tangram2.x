...
test("window and document", function(){
	...
	equal( baidu.query( window ).height(), clientVal, "window" );
	equal( baidu.query( document ).height(), Math.max(clientVal, scrollVal), "document" );
});
...
function diff( el, number ){
	...
	equal(baidu.query(el).height(number).height(),
