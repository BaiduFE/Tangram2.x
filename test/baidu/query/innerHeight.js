module('baidu.query.innerHeight');

test("baidu.query(el).innerHeight()", function(){
	testGet( "div" );
	testGet( "span" );//如何改
	testGet( "body" );
	testGet( "input" );
...
function testGet( tag ){
	var el = create( tag );
	equal( baidu.query( el ).innerHeight(),
       el.offsetHeight - style(el, 'borderTopWidth') - style(el, 'borderBottomWidth'), "check " + tag + " innerHeight" );
//    equal( baidu.query( el ).innerHeight(), 60, "check " + tag + " innerHeight" );
