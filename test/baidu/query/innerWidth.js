module('baidu.query.innerWidth');

test("baidu.query(el).innerWidth()", function(){
	testGet( "div" );
	testGet( "span" );//如何改
	testGet( "body" );
	testGet( "input" );
...
function testGet( tag ){
	var el = create( tag );
	baidu.query(el).innerWidth();
	equal( baidu.query( el ).innerWidth(),
	   el.offsetWidth - style(el, 'borderLeftWidth') - style(el, 'borderRightWidth'), "check " + tag + " innerWidth" );
//    equal( baidu.query( el ).innerWidth(), 60, "check " + tag + " innerWidth" );
