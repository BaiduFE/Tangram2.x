...
test('没有next情况的测试', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query('#e5').nextAll();
...
test('不重叠情形', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query("#e3, #e8").nextAll();
...
test('重叠情形', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query("#e2, #e3").nextAll();
		equal(target.length, 3);
...
test('.nextAll( selector )重载', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query('#e2, #e7').nextAll('p');
