...
test('没有next情况的测试', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query('#e5').nextUntil(".foo");
...
test('不重叠情形', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query("#e2, #e8").nextUntil('script');
...
test('重叠情形', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query("#e2, #e3").nextUntil('div');
		equal(target.length, 2);
		equal(target[0].id, 'e3');
