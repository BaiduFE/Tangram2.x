...
test('div event trigger once', function(){
    expect(5);
    var c = new Div();
    baidu.query(c.get()).one('click', function(){
    ...
    //
    c = new Div();
    baidu.query(c.get()).one('keyup', {tangId: 'Tangram'}, function(evt){
    ...
    c = new Div();
    baidu.query(c.get()).one(map, {tangId: 'Tangram'}, function(){ok(false, 'exception');});
    ua.fireMouseEvent(c.get(), 'click');
    ua.fireMouseEvent(c.get(), 'click');
...
test('span event trigger once', function(){
    ...
    baidu.query(span).one('click', function(){ok(true, 'click event trigger')});
    ua.fireMouseEvent(c.get(), 'click');
    ua.fireMouseEvent(c.get(), 'click');
...
//    
    ...
    c = new Div();
    span.appendChild(c.get());
    baidu.query(span).one('click', {tangId: 'Tangram'}, function(evt){
...
//    
    ...
    c = new Div();
    span.appendChild(c.get());
    baidu.query(span).one(map, {tangId: 'Tangram'}, function(){ok(false, 'exception')});
...
//     expect(3);
//     var c = new Div(true);
//     baidu.query(c.get()).one('mousedown', function(){
...
//     c = new Div(true);
//     baidu.query(c.get()).one('click', {tangId: 'Tangram', tangNum: 100}, function(evt){
...
test('off event', function(){
    ...
    baidu.query(c.get()).one('click', handler);
    baidu.query(d.get()).one('click', handler);
    baidu.query(c.get()).off('click', handler);
    ua.fireMouseEvent(c.get(), 'click');
