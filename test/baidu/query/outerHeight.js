module('baidu.query.outerHeight');

test("baidu.query(el).outerHeight()", function(){
	testGet( "div" );
	testGet( "span" );//如何改
	testGet( "body" );
	testGet( "input" );
...
function testGet( tag ){
	var el = create( tag );
	equal( baidu.query( el ).outerHeight(), el.offsetHeight, "check " + tag + " outerHeight()" );
	equal( baidu.query( el ).outerHeight( true ), el.offsetHeight + style(el, 'marginTop') + style(el, 'marginBottom'), "check " + tag + " outerHeight( true )" );
	
	
	
//    equal( baidu.query( el ).outerHeight(), 100, "check " + tag + " outerHeight()" );
//    equal( baidu.query( el ).outerHeight( true ), 140, "check " + tag + " outerHeight( true )" );
