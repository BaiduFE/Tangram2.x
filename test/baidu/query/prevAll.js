...
test('没有prev情况的测试', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query('#e5').prevAll();
...
test('不重叠情形', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query("#e3, #e8").prevAll();
...
test('重叠情形', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query("#e3, #e4").prevAll();
		equal(target.length, 2);
		equal(target[0].id, 'e2');
...
test('.prevAll( selector )重载', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query('#e4, #e6').prevAll('p');
		equal(target.length, 2);
		equal(target[0].id, 'e2');
		equal(target[1].id, 'e3');
	});
