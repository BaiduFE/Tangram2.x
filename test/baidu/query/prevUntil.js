...
test('没有prev情况的测试', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query('#e5').prevUntil(".bar");
...
test('不重叠情形', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query("#e4, #e9").prevUntil('script');
...
test('重叠情形', function () {
	useTangramDom(html, function ($dom, wrapper) {
		var target = baidu.query("#e3, #e4").prevUntil('script');
