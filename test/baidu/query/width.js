...
test("window and document", function(){
	...
	equal( baidu.query( window ).width(), clientVal, "window" );
	equal( baidu.query( document ).width(), Math.max(clientVal, scrollVal), "document" );
});
...
test("display:none", function(){
	...
	equal( baidu.query( div ).width(), 100, "display:none;" );
	div.parentNode.removeChild( div );

	var div = create( "div" );
	div.style.width = "100px";
	div.style.visibility = "hidden";
	equal( baidu.query( div ).width(), 100, "visibility:hidden;" );
	div.parentNode.removeChild( div );
});
...
function diff( el, number ){
	...
	equal( baidu.query( el ).width( number ).width(),
...
//    equal( baidu.query( el ).width( number ).width(), isNaN( number ) ? 0 : Math.max( number, 0 ), "针对 " + el + " 节点设置(" + number + ")和取得 width");
