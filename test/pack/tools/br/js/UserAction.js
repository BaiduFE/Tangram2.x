 ...
 */
{
	...
	commonData : {// 针对测试文件的路径而不是UserAction的路径
		...
		datadir : (function() {
			var href = '';
			if(location.href.indexOf("/run.do") > 0) {
				href = location.href.split("/run.do")[0] + "/test/pack/tools/data/";
			} else {
				href = location.href.split("/test/")[0] + "/test/pack/tools/data/";
			}
			return href;
		})(),
		currentPath : function() {
			...
			for ( var i = 0; i < params.length; i++) {
				...
				if (p.split('=')[0] == 'case') {
					...
					var href = '';
					if(location.href.indexOf("/run.do") > 0) {
						href = location.href.split('/run.do')[0] + '/test/pack/'
						+ casepath.substring(0, casepath.lastIndexOf('/'))
						+ '/';
					} else {
						href = location.href.split('/test/')[0] + '/test/pack/'
						+ casepath.substring(0, casepath.lastIndexOf('/'))
						+ '/';
					}
					return href;
